...
**Estado: TALLER COMPLETO — 37 secciones (las 36 de la secuencia de abajo + [Serverless (Azure Functions)](secciones/serverless-azure-functions.md), intercalada tras [Diseñar con eventos](secciones/disenar-con-eventos.md)).** Bloques A convergencia · B revelar Critter Stack · C EDA · D CQRS · E multi-tenancy · F testing/idioma/NuGet · G capstone. Cada sección verificada por Bash y validada con los agentes (coherencia/técnico/principiante; build-along donde aplica); el código de framework **compila contra paquetes reales** (Marten 9.10 en [Revelar Marten](secciones/revelar-marten.md), WolverineFx 6.14 en [Revelar Wolverine](secciones/revelar-wolverine.md) y [Transportes (RabbitMQ y ASB)](secciones/transportes-rabbitmq-asb.md), proyección Marten 9 en [CQRS y proyecciones](secciones/cqrs-proyecciones.md), test xUnit verde en [Given-When-Then](secciones/given-when-then.md)) y se **verificó e2e con Docker** (Postgres + RabbitMQ) que [Revelar Marten](secciones/revelar-marten.md)/[Revelar Wolverine](secciones/revelar-wolverine.md)/[Transportes (RabbitMQ y ASB)](secciones/transportes-rabbitmq-asb.md) corren contra infra real. Cada sección ancla 1:1 a `Cosmos.BuildingBlocks` (línea 🔗 con el tipo real al que converge). 0 errores de estructura.

> **Orden y numeración.** Las secciones ya **no llevan número** en el archivo (son *slugs*, p. ej. `el-almacen-por-id.md`); el orden lo definen **este mapa** (la secuencia de abajo) y la cadena ⬅️/➡️. Las citas entre secciones son **enlaces por título**, así que insertar, quitar o reordenar **no rompe referencias** — solo se ajustan los ⬅️/➡️ vecinos y esta lista.
...
- **🛠️ Construyes (reto):** el envoltorio `EventStreamDeEmpresa`, **dueño de su propia lista**, con `Append(hecho)` para **anotar** un hecho nuevo y `Get()` para crear una `Empresa` vacía y rehidratarla con `Load`.
- **💡 Qué aprendes:** el patrón **Repositorio** (esconder *cómo* se guarda y se rehidrata; el consumidor solo `Append`/`.Get()`); qué es un **Event Stream** (la historia de UNA empresa); y que el stream **lee y escribe** (cerrando el "anotar el diario" de Los primeros hechos).
...
## El despachador
- **🛠️ Construyes (3 retos):** al querer **un punto único que rutee cualquier comando a su handler**, nacen las piezas que eso exige: (1) un **`record` por comando** (`CambiarPlanDeEmpresa`, …) para poder rutear por **tipo** —y reescribir los handlers de `Handle(string)` a `Handle(comando)`—; (2) el contrato común **`ICommandHandler<T>`** que implementan todos; (3) el **`Despachador`** (`Dictionary<Type, Action<object>>` + `Registrar<T>` + `Enviar(object)`, ruteando por `comando.GetType()`, sin `switch`/`dynamic`/reflexión).
- **💡 Qué aprendes:** el patrón **mediador / dispatcher**; y —el corazón— **por qué existen el record y `ICommandHandler<T>`**, introducidos en su **momento de necesidad**: el record le da a cada intención una identidad de **tipo** (la llave del ruteo; con `string` colisionarían), y la interfaz es el **contrato común** sin el cual un despachador no puede tratar a todos los handlers por igual (`handler.Handle((T)c)` no se escribe sin él).
- **👁️ Se te muestra:** el truco de la lambda que captura `T` y "borra" el genérico a un `Action<object>` uniforme. *(Por qué: el cierre sobre el parámetro genérico es sutil.)*
- **🔗 Ancla:** converge al despacho por tipo que Wolverine industrializa (`Discovery.IncludeAssembly` ≈ tu `Registrar`; `IMessageBus.InvokeAsync` ≈ tu `Enviar`) — al descubrir por convención, `ICommandHandler<T>` queda **opcional**. El `record`-comando y el contrato son los que la plantilla expone (`ICommand`/`ICommandHandler<T>`). El `Despachador` es andamio: lo reemplaza Wolverine.
- **Tu calificación:** [ ] _____

## El almacén: un cajón por empresa
- **🛠️ Construyes (reto):** el `EventStore` — un `Dictionary` con un cajón de hechos por empresa (por id), `GetEvents(id)`/`AppendEvent(id, hecho)`; el `Id` en `AggregateRoot`; el `EventStream` deja de guardar su lista y **delega** en el almacén (vía `AbrirStream<T>`, la fábrica que se pasa `this`); el comando recupera su `EmpresaId` y el handler **busca por id**.
- **💡 Qué aprendes:** el patrón **Event Store** (almacén central por id) — nace de un dolor **de código**: los handlers atados a un stream **no escalan a la 2ª empresa** (colisionan por tipo en el despachador); que **el id es la llave** (y viaja solo: llave del cajón, parámetro del stream, identidad del agregado); handlers registrados **una vez** para todas.
- **👁️ Se te muestra:** el idioma nuevo en cajas 🆕 (`new(this, …)` target-typed, `new T { Id = }`, la restricción en un **método** genérico). El `EventStream`/`AbrirStream` va como **reto** (hints + solución en `<details>` + un `🔍` intermedio), no como bloque cerrado.
- **Tu calificación:** [ ] _____

## Cuando dos escriben a la vez (concurrencia optimista)
- **🛠️ Construyes (reto):** el **sobre `EventoAlmacenado(Version, …)`** (numerar cada hecho con su posición); el `EventStore` pasa a guardar **sobres** y **valida la versión** en `AppendEvent` (lanza `ConcurrencyException` si la posición ya está ocupada); el `EventStream` numera al escribir y **desenvuelve** al leer.
- **💡 Qué aprendes:** el dolor de la **actualización perdida** (dos escritores simultáneos que se pisan en silencio); la **concurrencia optimista** (no bloquear; verificar la versión al guardar y reintentar). El sobre nace **junto a su dolor**, no antes.
- **👁️ Se te muestra:** `.Select(...)` (desenvolver el sobre) en caja 🆕; el resto, reto con solución en `<details>`.
- **Tu calificación:** [ ] _____
...
## [Adoptar una herramienta](secciones/por-que-adoptar-herramientas.md)+ · Lo que viene (planeado al detalle)

> Cada sección, en el mismo formato que [El diario de una empresa](secciones/el-diario-de-una-empresa.md) a [Docker y PostgreSQL](secciones/docker-postgres.md), **más una línea 🔗 Ancla**: el tipo REAL de `Cosmos.BuildingBlocks` al que lo construido converge 1:1 (para que la "revelación" sea renombrar/intercambiar, no rediseñar — la regla de oro del METODO). Las divergencias con la doc oficial de Marten/Wolverine van marcadas como **⚖️**.

---

### 🅰️ Bloque A — Adopción y convergencia (cierra el motor a mano y lo lleva a la forma del canon)

> Esta es la **convergencia 4b** (obligatoria antes de revelar): el motor de [Los primeros hechos](secciones/los-primeros-hechos.md) a [Inyección de Dependencias](secciones/inyeccion-de-dependencias.md) se refactoriza, paso a paso, hasta tener la forma exacta de la plantilla. Varias secciones **reescriben** código ya hecho.

## La bisagra de adopción
...
## El agregado que acumula (de `decide`-devuelve a `Raise`-acumula)
- **🛠️ Construyes (reto):** que `Empresa` deje de **devolver** el hecho desde `decide` ([Decidir el futuro](secciones/decidir-el-futuro.md)) y en su lugar lo **levante**: una lista de no-confirmados + un `Raise(hecho)` que lo encola. Los métodos de negocio llaman a `Raise`.
- **💡 Qué aprendes:** el modelo de producción — el agregado es **dueño de sus eventos pendientes** (uncommitted), no una función que retorna. *(Reescribe [Decidir el futuro](secciones/decidir-el-futuro.md).)*
...
## Aplicar al levantar (`Raise` también aplica + `Apply` por tipo)
- **🛠️ Construyes (reto):** que `Raise`, además de encolar, **aplique** el hecho llamando al dispatcher (`Aplicar`), para que el estado refleje la decisión **al instante**; y **partir** el `switch Aplicar(object)` para que cada `case` solo **enrute** a un método **`Apply(TEvento)` público** (donde vive la mutación). Cierra la semilla de [El agregado que acumula](secciones/el-agregado-acumula.md).
...
- **👁️ Se te muestra:** el despacho por **`switch` tipado** que enruta a los `Apply(T)` (NO reflexión, NO `dynamic` — cashea la "otra forma" de [Refactorizando el motor](secciones/refactorizando-el-motor.md)).
- **🔗 Ancla:** converge al patrón de `FakeAggregateRoot`: encolar+aplicar (su `RaiseAndApply`/`ApplyInternal`) + un `public void Apply(TEvento)` por evento (lo que Marten/`TestStore` llaman para rehidratar). **⚖️ Divergencia del `Id`:** la plantilla lo estampa dentro del `Apply` del evento de creación (su evento de creación **lleva** el id); nuestro diseño deja el evento **sin id** (id = llave del stream, [El almacén por id](secciones/el-almacen-por-id.md)) → se difiere a [Revelar Marten](secciones/revelar-marten.md). *(Simplificaciones didácticas: nuestro `Raise`/dispatcher viven en la base —[El agregado que acumula](secciones/el-agregado-acumula.md)—, la plantilla los pone per-agregado; no afecta el contrato de persistencia → el reveal sigue 1:1.)*
- **Tu calificación:** [ ] _____

## La versión es del agregado, no del stream
- **🛠️ Construyes (reto):** mover la `Version` que vivía en el sobre/stream ([Concurrencia optimista](secciones/concurrencia-optimista.md)) al propio agregado — una propiedad `Version` (`int`) de solo-lectura externa; el `Load` la sube al cargar (un hecho persistido a la vez); Marten la poblará al rehidratar.
- **💡 Qué aprendes:** `Version` es **metadato del stream** que el agregado expone, no lógica que el agregado calcula; es la base de la **concurrencia optimista**. *(Reescribe el sobre de [Concurrencia optimista](secciones/concurrencia-optimista.md).)*
...
- **🔗 Ancla:** la forma converge 1:1 a `IEventStore` + `IAggregateRootReader` (6 sobrecargas `GetAggregateRootAsync`) y al patrón `MartenUnitOfWork` (`_started`/`_modified`, `UncommitedModifiedAggregateRoots`, `ClearChangeTracker`). La **interfaz** `IEventStore` se extrae en [Revelar Marten](secciones/revelar-marten.md), al aparecer la 2ª implementación.
- **Tu calificación:** [ ] _____
...
## El gran reveal (1): Marten reemplaza tu almacén
...
- **👁️ Se te muestra:** Marten/Critter Stack — `IDocumentSession` (escritura) vs `IQuerySession` (lectura), `StreamIdentity.AsString` (para que la llave sea string como tu `Id`), registro `Scoped`. **⚖️ Caja de divergencia:** la plantilla rehidrata con `AggregateStreamAsync` + escribe con `Append` y **NO usa `FetchForWriting`** (la doc de Marten lo recomienda hoy) — *semilla* 🌱 de "las dos vertientes" ([Las dos vertientes](secciones/dos-vertientes.md)).
...
## El gran reveal (2): Wolverine reemplaza tu despacho + el commit automático
- **🛠️ Construyes (reto):** reemplazar el despacho/contenedor casero ([El Command Handler](secciones/el-command-handler.md) + [Inyección de Dependencias](secciones/inyeccion-de-dependencias.md)) por Wolverine — `UseWolverine` + `Discovery.IncludeAssembly` (descubre handlers **por convención**) + `IntegrateWithWolverine`; y extraer el "guardar al final" repetitivo a un **middleware** (`After()` appendea los pendientes, `Finally()` limpia) + `AutoApplyTransactions`.
...
## Levantar la última magia: reflexión vs codegen
- **🛠️ Construyes:** (poco a mano) contrastar tu despacho por diccionario/reflexión ([Inyección de Dependencias](secciones/inyeccion-de-dependencias.md)) contra el **código que Wolverine genera** (codegen) y el **SQL que emite Marten**.
...
- **🔗 Ancla:** ancla en `o.UseRuntimeCompilation()` (`WolverineExtensions`); se contrasta con el `TestStore` reflexivo (la reflexión **sí** es apropiada para tests, ver [El TestStore](secciones/teststore.md)).
- **Tu calificación:** [ ] _____

## Las dos vertientes de persistencia (nativo vs la capa de la plantilla)
...
- **🔗 Ancla:** cosecha 🌳 de la semilla de [Revelar Marten](secciones/revelar-marten.md); contrasta `MartenEventStore` (`AggregateStreamAsync`+`Append`, **0** `FetchForWriting` verificado) vs el camino nativo documentado.
- **Tu calificación:** [ ] _____
...
## Quién se lleva los hechos que salen: los senders + el sobre
...
## El proceso que se cae: por qué necesitas Outbox/Inbox
...
- **🔗 Ancla:** converge 1:1 a `UseDurableOutbox()`/`UseDurableOutboxOnAllSendingEndpoints()` y `HabilitarInbox()` = `UseDurableInboxOnAllListeners()` / `UseDurableInbox()`. El `SendInline()` de Azure Service Bus es el contraste (el camino **sin** outbox, ver [Transportes (RabbitMQ y ASB)](secciones/transportes-rabbitmq-asb.md)).
- **Tu calificación:** [ ] _____
...
- **🔗 Ancla:** converge 1:1 a `WolverineExtensions` de RabbitMQ (`ToRabbitExchange`/`ListenToRabbitQueue`/`UseDurableOutbox`/`UseDurableInbox`) y de Azure Service Bus **dockerizado** (`HabilitarAzureServiceBus`, `PublicarEvento` → `UseDurableOutbox`; brokers nombrados). El contraste **serverless** (`SendInline`) se trata a fondo en [Serverless (Azure Functions)](secciones/serverless-azure-functions.md). *(Puente al taller de microservicios/EDA.)*
- **Tu calificación:** [ ] _____

## Diseñar con eventos
- **🛠️ Construyes:** nada (sección de **diseño/criterio**, sin código nuevo).
- **💡 Qué aprendes:** las decisiones que el broker **NO** toma por ti, sobre un caso real (`RegistroObligacionCreado → TerceroCreado/TerceroRechazado`): **granularidad de topics** (por-servicio + ruteo por tipo, no por-evento); **evento vs comando** (si esperas respuesta es un request → gRPC, no un evento disfrazado = acoplamiento temporal); **acoplamiento** (dependencia circular A→B→A, *smell* ≠ antipatrón); **coreografía vs orquestación**.
- **👁️ Se te muestra:** el criterio para **razonar** EDA, no más mecánica. *(Por qué: separa "cablear EDA" de "diseñar con EDA".)*
- **🔗 Ancla:** siembra el taller **⑤ Microservicios** (sagas, orquestadores, transacciones distribuidas, comunicación/resiliencia entre servicios) — **enlace entre talleres** al índice de la academia.
- **Tu calificación:** [ ] _____

## Serverless: la plantilla en Azure Functions
- **🛠️ Construyes (diagnóstico + configurar):** despliegas tu app de [Outbox e Inbox](secciones/outbox-inbox.md) a [Transportes (RabbitMQ y ASB)](secciones/transportes-rabbitmq-asb.md) (host largo + outbox durable) como **Azure Function** y se rompe: (a) Wolverine no arranca y (b) los eventos publicados nunca salen. Diagnostica por qué y aplica las **3 variantes serverless**: host con `AddWolverine(ExtensionDiscovery.ManualOnly)` (no `UseWolverine`), `DurabilityMode.Solo`, y enviar con `SendInline()` en vez de `UseDurableOutbox()`.
...
## El lado de lectura: proyecciones y consultas (CQRS de verdad)
...
## Versionado y upcasting de eventos
...
## ¿De dónde sale el tenant? No lo pidas, resuélvelo
...
## El tenant viaja con el mensaje (cuando ya no hay request)
...
## Probar el motor sin base de datos: el `TestStore`
...
## Given-When-Then: una gramática para probar decisiones
...
## Empaquetar y publicar el motor como NuGet
...
## Capstone: un bounded context de la `Empresa`, de punta a punta
