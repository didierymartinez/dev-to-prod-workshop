...
## §13 · Aplicar al levantar: `RaiseAndApply` + `Apply(TipoConcreto)`
- **🛠️ Construyes (reto):** que `Raise`, además de encolar, **aplique** el hecho al estado: un `RaiseAndApply(hecho)` que encola y despacha por tipo a un `Apply(TEvento)` por evento. El `Id` se estampa **dentro** de `Apply(EmpresaRegistrada)`, no en el constructor; el constructor queda **sin parámetros**.
- **💡 Qué aprendes:** la **doble naturaleza** de `Apply` — la misma lógica (a) avanza el estado al ejecutar un comando y (b) reconstruye el estado al reproducir el stream; por qué el ctor es vacío y el estado solo cambia vía `Apply`.
- **👁️ Se te muestra:** el despacho por **`switch` sobre tipo concreto** (NO reflexión: más rápido y explícito) — y que la base `AggregateRoot` queda **deliberadamente mínima** (no trae `Apply`/`Raise`; cada agregado los implementa).
- **🔗 Ancla:** converge 1:1 al patrón de `FakeAggregateRoot` (`Cosmos.EventSourcing.CritterStack.Tests`): `RaiseAndApply(object){ _uncommittedEvents.Add; ApplyInternal; }` + `ApplyInternal` con `switch` por tipo + un `public void Apply(TEvento)` por evento, con el `Id` en `Apply` del evento de creación.
- **Tu calificación:** [ ] _____

## §14 · La versión es del agregado, no del stream
- **🛠️ Construyes (reto):** mover la `Version` que vivía en el sobre/stream (§05) al propio agregado — una propiedad `Version` (`int`) de solo-lectura externa; el almacén la cuenta/puebla al cargar.
...
- **👁️ Se te muestra:** por qué el `set` es protegido y la rellena el almacén (luego Marten).
...
- **🔗 Ancla:** converge 1:1 a `WolverineExtensions` de RabbitMQ (`ToRabbitExchange`/`ListenToRabbitQueue`/`UseDurableOutbox`/`UseDurableInbox`) y de Azure Service Bus (`UseDurableOutbox` vs `SendInline`; brokers nombrados). *(Puente al taller de microservicios/EDA.)*
- **Tu calificación:** [ ] _____
