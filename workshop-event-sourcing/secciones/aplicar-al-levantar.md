...
> [!NOTE]
> 🌱 **Semilla — dónde nace el `Id` (solo un adelanto; no cambies nada).** Tu `Apply(EmpresaRegistrada)` está **completo y correcto así, sin tocar el `Id`**: en nuestro diseño el id es la **llave del stream** ([El almacén por id](el-almacen-por-id.md)) y el evento **no lo lleva** — el agregado recibe su id **al crearse o cargarse**, no desde el evento (lo verás en [El almacén directo](el-almacen-directo.md)). La herramienta que adoptaremos ([Revelar Marten](revelar-marten.md)) toma el otro camino —su evento de creación **carga el id** y lo estampa dentro de su `Apply`—. Las dos son válidas; cuál encaja mejor con Marten lo veremos al revelarlo.
