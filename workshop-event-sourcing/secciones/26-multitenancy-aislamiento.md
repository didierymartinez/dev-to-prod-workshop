...
Tu almacén guarda streams con la llave = **id del agregado** (§05). Pero la plataforma sirve a **muchas** empresas-cliente, y los datos de una **no** deben mezclarse con los de otra. A cada cliente lo llamamos un **tenant**. Y aquí hay una trampa: ¿qué pasa si dos tenants tienen una empresa con el **mismo** id?
...
> [!NOTE]
> 🌱 **Semilla — pero, ¿de dónde sale el `tenantId`?** Ahora mismo lo pasas a mano en cada llamada. Eso ensucia todo. En §27 lo esconderás tras una mini-interfaz ("dime el tenant actual") y, en §28, harás que **viaje con el mensaje** cuando no haya una petición HTTP de por medio.
...
[⬅️ Volver: Versionado y upcasting](./25-versionado-upcasting.md)

[➡️ Siguiente: ¿De dónde sale el tenant? (resolverlo, no pedirlo)](./27-resolver-el-tenant.md)
