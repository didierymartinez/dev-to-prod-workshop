...
| **Eventos públicos vs privados** | el `AggregateRoot` expone `GetPublicEvents`/`GetPrivateEvents` para rutear a EDA (RabbitMQ/ASB); el nativo no separa eso | [Público vs privado](publico-vs-privado.md) a [Transportes (RabbitMQ y ASB)](transportes-rabbitmq-asb.md) |
| **Portabilidad y control** | el dominio depende de **tu** `IEventStore`, no de Marten directo; controlas el Unit of Work y la multi-tenancy | [El almacén directo](el-almacen-directo.md), [Multi-tenancy](multitenancy-aislamiento.md) a [El tenant viaja con el mensaje](tenant-viaja-con-el-mensaje.md) |
...
- **La concurrencia optimista es TU responsabilidad.** El nativo `FetchForWriting` la da gratis; tú la hiciste a mano ([El almacén directo](el-almacen-directo.md), con `ar.Version`). Y ojo: la capa del equipo, tal como está, **no la hace cumplir** en escritura — si la necesitas, la pones tú.
...
[⬅️ Volver: Levanta la última magia (reflexión vs codegen)](./reflexion-vs-codegen.md)

[➡️ Siguiente: Dos tipos de hecho (público vs privado)](./publico-vs-privado.md)
