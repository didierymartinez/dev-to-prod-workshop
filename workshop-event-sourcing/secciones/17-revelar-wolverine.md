# 17 · El gran reveal (2): Wolverine y el commit automático

Marten reemplazó tu almacén (§16). Falta la otra mitad del Critter Stack: **Wolverine**, que reemplaza dos cosas que hiciste a mano — el **despacho** de comandos (§07) y el contenedor que arma handlers (§09) — y, de regalo, te quita el `SaveChangesAsync` de encima.
...
2. **El despacho lo armas tú:** en §07 escribiste el `ICommandHandler<T>` y en §09 un mini-contenedor que resuelve e invoca. Es plomería: encontrar el handler del comando y llamarlo.
...
> 🛠️ **Inténtalo tú.** **🔁 Ajusta** tus handlers (los que ya implementan `ICommandHandler<T>` con el `IEventStore` por **constructor**, de §16): déjalos **cargar** (o `StartStream`) y **decidir** — y **borra el `await store.SaveChangesAsync(ct)`**. En un momento veremos quién lo llama por ti.
...
> [!NOTE]
> **Wolverine descubre tu `HandleAsync` por convención** — no le hace falta que implementes `ICommandHandler<T>` para encontrarlo (escanea métodos `Handle`/`HandleAsync`). Pero **conservamos** la interfaz de §07: documenta la intención y, sobre todo, te deja **probar el handler genéricamente** en §30. El `IEventStore` sigue inyectado por **constructor** (vía DI). Quien envía el comando hará `bus.InvokeAsync(comando)` y Wolverine resuelve el handler correcto.
...
builder.Services.AddScoped<IEventStore, MartenEventStore>();   // el swap de §16 SIGUE: así Handle(…, IEventStore store) resuelve
```

Esto reemplaza tu §07 + §09 + el `SaveChangesAsync` de cada handler:
- `Discovery.IncludeAssembly` → encuentra los handlers (adiós mini-contenedor).
- `UseRuntimeCompilation` → genera el código del despacho en el arranque (lo levantamos en §18).
- `IntegrateWithWolverine` → deja que Wolverine guarde sus propios mensajes en la **misma** transacción de Marten (el outbox/inbox; a fondo en §22).
...
> [!NOTE]
> 🌱 **Semilla — ¿magia? No: código generado (§18).** Wolverine no usa reflexión en cada llamada para encontrar tu `Handle`: **genera código** en el arranque (`UseRuntimeCompilation`) que llama tu handler y tu middleware directo. En §18 levantamos esa última magia y miramos el código que genera.
...
[⬅️ Volver: El gran reveal — Marten](./16-revelar-marten.md)

[➡️ Siguiente: Levanta la última magia (reflexión vs codegen)](./18-reflexion-vs-codegen.md)
