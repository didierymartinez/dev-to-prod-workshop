...
El `EventStream<T>` maneja muy bien la historia de **una** empresa. Pero quedó un cuello de botella: si tienes 1.000 empresas, tendrías 1.000 listas sueltas flotando en el programa. Falta un **almacén central** que las guarde a todas.

## 🎯 El Objetivo

Un único almacén que guarde y entregue la historia de **cualquier** empresa por su id, y que el `EventStream` sea el objeto con el que **lees y escribes** la historia de una de ellas.

## Cada empresa, su id

Para guardar muchas historias juntas, primero hay que poder **distinguirlas**: cada empresa necesita un **`Id`**. Hasta §04 una empresa era solo su estado —no le hacía falta—; ahora que vamos a guardar miles, el id es imprescindible. Se lo damos a la clase base:
...
¿Qué es el `EventStream`? Es el **objeto con el que trabajas la historia de una empresa**: lees sus hechos (`Get`) y escribes uno nuevo (`Append`). Y el enlace con el almacén es más simple de lo que parecía: **tú no lo construyes ni le pasas el almacén**. Se lo **pides** al almacén, y él te lo entrega ya conectado:
...
```csharp
// 🔁 Reemplaza el EventStream<T> de la §04: ya no guarda una lista; habla con el almacén
...
Escribimos y leemos **siempre por el stream** (los hechos aún los fabricamos a mano; la empresa aprenderá a emitirlos en §06):
