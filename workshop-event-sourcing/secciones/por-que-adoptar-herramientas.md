# El momento de adoptar una herramienta

Construiste, a mano y desde cero, un motor de Event Sourcing **que funciona**: guarda hechos, rehidrata agregados, decide con reglas, orquesta comandos, no bloquea hilos y se ensambla solo. En [Docker y PostgreSQL](docker-postgres.md) lo dejaste a las puertas de una base de datos real… y te topaste con un **muro** (recuperar un `object` como su tipo exacto, la concurrencia en SQL, los índices). Antes de escalar ese muro ladrillo por ladrillo, hay una decisión de ingeniería que tomar.
...
|---|---|
| **Serialización polimórfica** (`object` → su tipo) | el muro de [Docker y PostgreSQL](docker-postgres.md): un `switch` gigante y frágil sobre nombres de tipo, evento por evento |
...
- **Marten** — convierte **PostgreSQL** en un **event store** (y base documental) de primera. Guarda streams de eventos, los rehidrata reproduciéndolos, lleva la versión, serializa y **deserializa al tipo correcto** por ti, y trae proyecciones, multi-tenancy y time-travel horneados. Es tu `EventStore` y toda la fontanería de Postgres de [Docker y PostgreSQL](docker-postgres.md), hecho por expertos.
- **Wolverine** — un **mediador** que además hace **mensajería**. Un mediador recibe un comando y lo entrega a quien sabe manejarlo, sin que el emisor conozca al handler. Es justo lo que hace tu `Despachador` de [El despachador](el-despachador.md). Encuentra tus handlers, despacha comandos, **comitea (guarda) por ti** al final de cada uno, y publica eventos a un **broker** con **outbox/inbox** durable. Un broker es un servicio de mensajería intermedio, tipo cola, que aún no construyes. Es tu `CommandHandler` y tu orquestación, industrializados.
...
> 1. **Sentiste la necesidad** — chocaste con el dolor real (el muro de [Docker y PostgreSQL](docker-postgres.md), la concurrencia, el outbox), no la adoptas "porque está de moda".
...
1. **Afinar tu motor a su forma de producción** ([El agregado que acumula](el-agregado-acumula.md) a [El almacén directo](el-almacen-directo.md)): unos refactores guiados que llevan tu agregado, tu versión y tu almacén a la **forma exacta** que usan las herramientas del equipo — sintiendo, en el camino, los últimos dolores que justifican esa forma.
2. **Revelar las herramientas** ([Revelar Marten](revelar-marten.md)+): cuando tu motor ya tenga esa forma, enchufar Marten y Wolverine será **renombrar e intercambiar 1:1**, no rediseñar. Verás que "lo que escribiste, la librería lo hace".
...
- [ ] Explica por qué vamos a **afinar el motor a mano ([El agregado que acumula](el-agregado-acumula.md) a [El almacén directo](el-almacen-directo.md)) antes** de enchufar las herramientas.
...
[➡️ Siguiente: El agregado que acumula](./el-agregado-acumula.md)
