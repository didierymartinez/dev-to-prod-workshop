...
Tomas tu configuración de §22–§23 (host largo, `UseDurableOutbox`) y la subes a una Azure Function. Dos sorpresas:
...
> 🛠️ **Inténtalo tú (sin código).** Recuerda cómo funciona el **outbox durable** (§22): el evento se guarda en una tabla en la misma transacción, y **un proceso de fondo** lo lee y lo envía al bus, reintentando si hace falta. Ahora piensa: en una función que **se apaga apenas responde**, ¿quién corre ese proceso de fondo?
...
```csharp
// gemelo serverless de tu UsarWolverineParaComandos (§17)
builder.Services.AddWolverineRuntimeCompilation();   // del paquete WolverineFx.RuntimeCompilation (§17); aquí sobre IServiceCollection, no opts.UseRuntimeCompilation()
...
| | Host largo (Docker, §16–§23) | Serverless (Azure Functions) |
|---|---|---|
...
[⬅️ Volver: Transportes reales (RabbitMQ y Azure Service Bus)](./23-transportes-rabbitmq-asb.md)

[➡️ Siguiente: El lado de lectura (CQRS)](./24-cqrs-proyecciones.md)
