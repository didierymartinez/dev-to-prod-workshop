# El proceso que se cae: Outbox e Inbox

En [Senders y el sobre](senders-y-sobre.md) publicas los hechos públicos con un sender. Pero hay una grieta de fondo: tu handler hace **dos** escrituras a sistemas distintos — guarda el hecho en Postgres (Marten) **y** lo publica al bus. ¿Qué pasa si el proceso **se cae entre las dos**?
...
Son **dos** sistemas: la BD y el broker. No hay forma de escribir en ambos de forma **atómica** (todo o nada). Si confirmas la BD y luego el broker está caído o el proceso muere, los datos quedan **inconsistentes**. Al revés es peor: publicar antes de confirmar y que la confirmación falle → anunciaste un hecho que **no pasó**.

El truco clásico: convertir las dos escrituras en **una**. El mensaje a publicar se guarda en la **misma transacción** que el hecho, en una tabla **outbox** ("bandeja de salida"). Una transacción es un grupo de operaciones que pasan todas o ninguna. Luego, un **relay** (un proceso de fondo) lee esa bandeja y publica, **reintentando** hasta lograrlo. Si el proceso muere, el mensaje sigue en la outbox: al revivir, el relay lo envía.
...
// el "broker" de juguete: en producción es RabbitMQ / Azure Service Bus (Transportes (RabbitMQ y ASB))
...
Lo bueno: no escribirás esto en producción. El `IntegrateWithWolverine()` que pusiste en [Revelar Wolverine](revelar-wolverine.md) **activa el outbox/inbox transaccional sobre Marten** — el mensaje pendiente se guarda en la **misma transacción** de Marten que tus eventos, y el relay de Wolverine lo envía. Se enciende por política:
...
> [!NOTE]
> 🌱 **Semilla — "durable" vs "inline".** `UseDurableOutbox` guarda el mensaje y lo envía con un **relay** durable (sobrevive a caídas). Existe el opuesto, `SendInline()` — enviar **en el acto**, sin outbox — que tiene sentido cuando **no hay** un proceso de fondo que drene: justo el caso **serverless** (Azure Functions), que verás en [Serverless (Azure Functions)](serverless-azure-functions.md). Por defecto, en un host largo, quieres el **durable**.
...
- [ ] Sabes que `IntegrateWithWolverine` ([Revelar Wolverine](revelar-wolverine.md)) ya activa esto sobre Marten, y que se enciende con `UseDurableOutbox`/`UseDurableInbox`.
...
[⬅️ Volver: Quién se lleva los hechos (senders y el sobre)](./senders-y-sobre.md)

[➡️ Siguiente: Transportes reales (RabbitMQ y Azure Service Bus)](./transportes-rabbitmq-asb.md)
