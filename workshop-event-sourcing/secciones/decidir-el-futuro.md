...
No hay que tocar el stream: ya es **simétrico** desde [El flujo de vida](el-flujo-de-vida.md) —`Get()` lee, `Append()` escribe— y desde [El almacén en memoria](el-almacen-en-memoria.md) ese `Append` le pone la `Version` al hecho y lo archiva en el almacén. Lo que faltaba era **quién produce** el hecho que se va a guardar. Hasta ahora lo fabricábamos a mano; desde esta sección, lo produce la **empresa**. El ciclo completo queda así:
