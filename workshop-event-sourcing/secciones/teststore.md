...
> [!NOTE]
> **Aquí la reflexión SÍ es la elección correcta** (lo prometido en [Reflexión vs codegen](reflexion-vs-codegen.md)). En producción Wolverine/Marten **generan código** por rendimiento; pero en un test double el costo de arranque no importa y la **flexibilidad** de "busca el `Apply` que encaje" gana: cero `switch` que mantener, cero codegen. Cada herramienta donde encaja.
...
[⬅️ Volver: El tenant viaja con el mensaje](./tenant-viaja-con-el-mensaje.md)

[➡️ Siguiente: Given-When-Then (probar decisiones)](./given-when-then.md)
