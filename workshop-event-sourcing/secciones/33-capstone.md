# 33 · Capstone: un bounded context de punta a punta

Empezaste con una empresa que tenía **amnesia** (§01) y un `foreach` sobre cuatro variables (§02). Terminas con un motor de Event Sourcing completo, sobre la Critter Stack, que sabes **construir, explicar y mantener**. Es hora de juntarlo todo — y de revelar el secreto que guardamos todo el taller.
...
> 1. El agregado `Empresa`: acumula (`Raise`), aplica (`Apply(T)`), lleva su `Version`. (§12–§14)
> 2. Persistencia: el Unit of Work y el almacén directo (§15) → extraído a `IEventStore` y respaldado por **Marten** con `MartenEventStore` (§16).
> 3. Comandos y handlers (`ICommandHandler<T>`) descubiertos por **Wolverine**, con el middleware que comitea. (§17)
> 4. Eventos **públicos** (`IPublicEvent`) a un broker con **outbox** durable; **privados** al bus interno. (§20–§23)
> 5. Una **proyección** (`EmpresaResumen`) para las consultas (CQRS). (§24)
> 6. **Multi-tenancy**: la sesión por tenant, el `ITenantResolver` híbrido. (§26–§28)
> 7. La batería de tests **Given-When-Then** sobre el `TestStore`, sin base de datos. (§29–§30)
>
...
Por eso, cuando abras `Cosmos.BuildingBlocks`, no verás una caja negra: verás **tu propio motor**, hecho por el equipo. Lo puedes **leer**, **depurar** y **evolucionar** — y conoces las **dos vertientes** (§19) para decidir cuándo plegar una mejora de Marten/Wolverine a la capa propia o seguir el camino nativo.

## Las 7 preguntas que ahora sabes responder

1. ¿Por qué guardar el **diario** (eventos) y no la **foto** (estado)? (§01)
2. ¿Qué es `evolve`, `decide`, un Aggregate Root, un Event Store, un Unit of Work? (§02–§15)
3. ¿Cómo se mapea tu motor a **Marten** y **Wolverine**, y por qué el reveal fue 1:1? (§16–§17)
4. ¿Por qué la plantilla usa su **capa propia** en vez de `FetchForWriting` nativo, y qué cuesta? (§19)
5. ¿Cómo viaja un hecho **público** a otro servicio sin perderse (outbox/inbox)? (§20–§23)
6. ¿Cómo se aísla cada **tenant** y de dónde sale en HTTP vs en el daemon? (§26–§28)
7. ¿Cómo se **prueba** todo esto sin base de datos? (§29–§30)

## Hacia dónde seguir

- **Mantén la plantilla al día**: sigue la doc de Marten/Wolverine y traduce sus mejoras a la capa del equipo (§19). Recuerda los puntos donde la plantilla **diverge** (no usa `FetchForWriting`, no hace concurrencia optimista en escritura, los privados pueden ir a broker pese al README).
- **El taller de microservicios/EDA** profundiza el lado público (RabbitMQ/ASB, sagas, dead-letter) que aquí solo cruzaste.
- **El taller de DevOps** cubre el empaquetado/CI-CD (§32), los entornos y el despliegue —incluido serverless (§23b)— a fondo.
...
[⬅️ Volver: Empaquetar y publicar como NuGet](./32-nuget-empaquetado.md)
