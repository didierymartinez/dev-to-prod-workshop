...
public abstract class CommandHandlerTestBase
{
    ...
    protected readonly TestStore EventStore = new();   // el de §29
...
{
    protected abstract ICommandHandler<TCommand> Handler { get; }   // por eso conservamos ICommandHandler<T> (§17)
...
> [!NOTE]
> **Dos detalles para que compile.** (1) `new CambiarPlanHandler(EventStore)` funciona porque desde §16/§17 tu handler recibe **`IEventStore`** (no `InMemoryEventStore`), y el `TestStore` *es* un `IEventStore` — por eso le puedes pasar el doble de test. (2) `[Fact]` es el atributo de **xUnit** que marca un método como un test que el runner ejecuta.
...
> [!NOTE]
> ⚖️ **Es `CommandHandlerTestBase` + `CommandHandlerTest`/`CommandHandlerAsyncTest` de la plantilla, 1:1.** Y trae un verbo más: `ThenIsPublishedPublicly(...)`/`ThenIsPublishedPrivately(...)`, que afirma los **eventos publicados** usando los `TestPublicEventSender`/`TestPrivateEventSender` (el doble en memoria de §21). Así pruebas, sin broker, que un comando además **publicó** lo que debía. Por eso conservamos `ICommandHandler<T>` en §17: la base inyecta el `TestStore` al handler y lo ejecuta genéricamente.

> [!NOTE]
> 🌱 **Semilla — esto es posible porque el dominio es puro.** Pudimos probar todo —`decide`, `Apply`, el handler— **sin base de datos ni mocks** porque el agregado es una función pura (§02/§06) y el handler habla con `IEventStore` (intercambiable por el `TestStore`). Es el pago de las "dos vertientes" (§19): la capa del equipo cuesta más código, pero esto es lo que compra.
...
- [ ] Una `CommandHandlerTestBase` con `Given`/`Then`/`And` sobre el `TestStore` (§29).
...
- [ ] Sabes que la plantilla añade `ThenIsPublishedPublicly/Privately` (con los `Test…EventSender` de §21) para afirmar lo **publicado**.

---

## 🧠 En una frase

En Event Sourcing un test es **Given** (hechos previos) → **When** (un comando) → **Then** (los hechos emitidos), no filas de una tabla; una `CommandHandlerTestBase` sobre el `TestStore` (§29) lo hace legible con AwesomeAssertions (`BeEquivalentTo`), prueba el dominio **sin BD ni mocks** (porque es puro), y la plantilla añade `ThenIsPublished…` para afirmar lo publicado.

---

[⬅️ Volver: Probar sin base de datos (el TestStore)](./29-teststore.md)

[➡️ Siguiente: El idioma de la plantilla (extension members de C#14)](./31-extension-members.md)
